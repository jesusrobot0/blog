...
export function FancyLink({ href, text, dark }: Props) {
  return (
    <Link
      href={href}
      className={clsx(
        "overflow-hidden w-[258px] h-[52px] flex justify-center items-center gap-2 text-[18px] tracking-[.9px] uppercase border rounded-full group hover:border-transparent hover:bg-[#AD9C94] transition-colors duration-500",
        dark ? "border-[#fff]" : "border-[#121212]"
      )}
    >
      ...
      <div
        className={clsx(
          "flex justify-center items-center w-[30px] h-[30px] rounded-full group-hover:rotate-45 transition-transform duration-300",
          dark ? "bg-none" : "bg-white"
        )}
      >
        <svg
